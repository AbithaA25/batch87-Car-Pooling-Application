...
char *vechicle_model[VECHILE_MODEL_INDEX] = {"micro","sedan","suv","minivan"};
...
{                         //The register_transporter function starts here
       ...
       while(1)
        ...
        printf("Select the vechile model\n");

        for(int i =0 ; i<VECHILE_MODEL_INDEX ; i++)
                printf("%d for %s\n",i+1,vechicle_model[i]);

        while(1)
        {
                scanf("%d",&v_num);

                if(v_num >= 1 && v_num <=VECHILE_MODEL_INDEX)
                {
                        break;
                }
                else
        ...
        strcpy(new->type_of_v , vechicle_model[v_num-1]);
...
int transporter_to_list()
{
        ...
        FILE *p;
        tt t1;
        if((p=fopen("../data/Transporter","rb"))==NULL)
        {
...
int list_to_transporter()
{
        ...
        FILE *p1;
        if((p1=fopen("../data/Transporter","wb"))==NULL)
        {
